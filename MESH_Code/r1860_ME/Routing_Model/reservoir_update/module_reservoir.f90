...
!-    use model_dates
    ...
    type reservoir_f
        ...
        real*4 :: lat             ! Lat location of the reservoir (decimal degrees)
        real*4 :: long            ! Lon location of the reservoir (decimal degrees)
        real*4 :: SMAX            ! Max reservoir capacity (m3)
        real*4 :: flowO1          ! Initial discharge (m3 s-1)
        real*4 :: Intstor1        ! Initial storage (m3)
        real*4 :: qmaxmax         ! Downstream channel capacity
        real*4 :: inflowcorr      ! Inflow correction factor for evaporation
        real*4 :: deadst          ! Dead storage fraction of maximum storage ( 0.1 means 10% of max storage)
        real*4 :: RXN             ! Random term variance or range
        real*4 :: dsto(12)        ! Monthly min storage 12 values one per month (m3)
        real*4 :: nsto(12)        ! Monthly normal upper storage (m3)
        real*4 :: ndsto(12)       ! Monthly upper storage (m3)
        real*4 :: Qmin(12)        ! Monthly min release (m3 s-1)
        real*4 :: Qnor(12)        ! Monthly normal upper release (m3 s-1)
        real*4 :: Qnd(12)         ! Monthly upper release (m3 s-1)
        real*4 :: stoSIM(2)       ! storage time series of the reservoir
        real*4 :: flowSIM(2)      ! storage time series of the reservoir
        real*4 :: flowINF(2)      ! inflow time series of the reservoir
    end type
    ...
    real (kind=4) function S2Q1JAN(Fu, Ld, Lc, Ln, Lf, qmin, qnorm, qnd, dt, smax, Inflow, qds, cse, Rx) result(qout)
        ...
        !> Input variables.
        real*4, intent(in) :: Fu, Ld, Lc, Ln, Lf, qmin, qnorm, qnd, dt, smax, Inflow, qds, Rx
    ...
    subroutine SIMRES15(icor, cse, Ld, flowI, smax, qds, Lc, Ln, Lf, qmin, qnorm, qnd, Rx, t, niter, dt, flowSIM, stoSIM)
        ...
        !> Input variables.
        real*4, intent(in) :: icor                ! = resrv%inflowcorr
        integer, intent(in) :: cse              ! = resrv%modeltype             ! if cse=1 inflow is not used in zone 3, else it is used 
        real*4, intent(in) :: Ld                  ! = resrv%deadst
        real*4, dimension(:), intent(in) :: flowI ! = resrv%flowINF(1:2)          ! observed u/s inflow
        real*4, intent(in) :: smax                ! = resrv%SMAX                  ! maximum reservoir storage (m3)
        real*4, intent(in) :: qds                 ! = resrv%qmaxmax               ! downstream channel capacity (m3/s)
        real*4, intent(in) :: Lc                  ! = resrv%dsto(mId)/resrv%SMAX  ! monthly lower storage threshold
        real*4, intent(in) :: Ln                  ! = resrv%nsto(mId)/resrv%SMAX  ! monthly upper storage threshold
        real*4, intent(in) :: Lf                  ! = resrv%ndsto(mId)/resrv%SMAX ! inflow starts affecting release
        real*4, intent(in) :: qmin                ! = resrv%Qmin(mId)
        real*4, intent(in) :: qnorm               ! = resrv%Qnor(mId)
        real*4, intent(in) :: qnd                 ! = resrv%Qnd(mId)
        real*4, intent(in) :: Rx                  ! = 2*resrv%RXN*rnd + resrv%RXN ! Rx not inclued in the Matlab code, but added based on MESH code
        integer, intent(in) :: t                ! = t                           ! current time-step
        integer, intent(in) :: niter            ! = 40                          ! it was 1000 in the Matlab code but 40 in the MESH code
        real*4, intent(in) :: dt                  ! = dt                          ! time-step length/duration

        !> Local variables.
        real*4 stoflo, Inflow, Fu
        integer it

        !> Output variables.
        real*4, dimension(:), intent(out) :: flowSIM  ! = resrv%flowSIM(1:2)
        real*4, dimension(:), intent(out) :: stoSIM   ! = resrv%stoSIM(1:2)
    ...
    subroutine compute_reservoir(resrv, flowIn, t, dt, mId)
        ...
        !> Input variables.
        real*4, intent(in) :: flowIn  ! flow from downstream grid cell from routing
        integer, intent(in) :: mId  ! month to idenfy monthly variable parameters
        integer, intent(in) :: t    ! current time step
        real*4, intent(in) :: dt      ! time-step length/duration
        ...
        !> Local variables.
!-        integer            :: irsv, MT, i
!-        real*4               :: FU, LD, LC, LN, LF, RX, ICORR
        real*4 rnd
		real*4 Rx
...
!-        end if
        ...
        Rx = 2.0*resrv%RXN*rnd + resrv%RXN
