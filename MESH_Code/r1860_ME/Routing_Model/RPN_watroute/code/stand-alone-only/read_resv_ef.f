...
!            as given in the event file is used!!!!!!!!!1
     ...
     *        yres(noresv),xres(noresv),cfn(noresv), !nk 18/06/04 ME 27/04/2021
...
!     *        net_lake_outflow(noresv,nrel_max),
     ...
     *           resname(1),qrel(1,nrel),qdwpr(1,nrel),cfn(1),
     *           stat=iAllocate)
            if(iAllocate.ne.0) STOP
...
!         as only the values in the first event need to be adjusted.
      do i=1,noresv
         ...
         cfn(i) = colHeader%colCoeff8(i) ! cfn value
         !if(b3(i).eq.0.0) then
         !  poliflg(i)='n'
         !else
         !  poliflg(i)='y'
...
!           if (longrid(1).gt.0.0 .and. latgrid(1).gt.0.0) then
     ...
     *           b6(i),b7(i),cfn(i),resname(i),i=1,noresv)
         endif
...
! ME 21/5/2021 - changed to use cfn to explicitly define reservoir type - cfn = 1 for insertion        
         if(cfn(1).eq.1)then
...
! In that situation, the initializing value of qo2 is used.
         if(qrel(k,1).lt.0.0 .and. cfn(k).eq.1)
...
! FORMATS
 ...
 1011 format(' ',3x,'  i  ires(i) jres(i)    b1(i)     b2(i)',
     *     '    b3(i)     b4(i)    b5(i)   b6(1)    b7(i)    cfn(i)')
 1013 format(' ',3x,i3,2i8,7e12.3,i8,a12/)
