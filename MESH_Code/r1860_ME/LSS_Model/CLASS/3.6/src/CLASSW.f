...
C
     ...
     4     THLINV(ILG),   SNOROF(ILG),   WSNROF(ILG)
C
      INTEGER             LZFAV (ILG)
C
...
C           * Done only if PBSMFLAG is off as it is done in PPSMrun when it is on
C              IF (.not. PBSMFLAG) THEN                                                            
C                 IF(ZSNOW(I).GT.10.0) THEN                                 
C                   SNOROF(I)=(ZSNOW(I)-10.0)*RHOSNO(I)                      
C                   WSNROF(I)=WSNOW(I)*SNOROF(I)/SNO(I)                         
C                   TOVRFL(I)=(TOVRFL(I)*OVRFLW(I)+TSNOW(I)*(SNOROF(I)+      
C     1                      WSNROF(I))/DELT)/(OVRFLW(I)+(SNOROF(I)+   
C     2                      WSNROF(I))/DELT)                                       
C                   OVRFLW(I)=OVRFLW(I)+(SNOROF(I)+WSNROF(I))/DELT              
C                   TRUNOF(I)=(TRUNOF(I)*RUNOFF(I)+TSNOW(I)*(SNOROF(I)+      
C     1                      WSNROF(I))/DELT)/(RUNOFF(I)+(SNOROF(I)+   
C     2                      WSNROF(I))/DELT)                                       
C                   RUNOFF(I)=RUNOFF(I)+(SNOROF(I)+WSNROF(I))/DELT              
C                   ROFN(I)=ROFN(I)+(SNOROF(I)+WSNROF(I))/DELT                  
C                   PCPG(I)=PCPG(I)+(SNOROF(I)+WSNROF(I))/DELT                  
C                   HTCS(I)=HTCS(I)-TSNOW(I)*(SPHICE*SNOROF(I)+SPHW*         
C     1                    WSNROF(I))/DELT                                  
C                   SNO(I)=SNO(I)-SNOROF(I)                                  
C                   WSNOW(I)=WSNOW(I)-WSNROF(I)                              
C                   ZSNOW(I)=10.0                                         
C                 ENDIF

              IF (.not. PBSMFLAG) THEN               !DEAL WITH VANISHINGLY SMALL SNOWPACK 
