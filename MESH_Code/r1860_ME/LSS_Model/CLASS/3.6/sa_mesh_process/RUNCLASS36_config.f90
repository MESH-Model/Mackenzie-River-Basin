...
1118 format(3x, a, ': ', i6)
        ...
        allocate(cpv%ALBS(NML), cpv%CMAI(NML), cpv%GRO(NML), cpv%QAC(NML), cpv%VAC(NML), cpv%RCAN(NML), cpv%RHOS(NML), cpv%SNCAN(NML), &  ! ME
...
!todo: move to read_parameters
    ...
    subroutine read_init_prog_variables_class_row(fls, shd)
        ...
        !> For SAVERESUMEFLAG 3, ME added VACROW for canopy level wind speed
        real(kind = 4), dimension(:, :), allocatable :: ALBSROW, CMAIROW, GROROW, QACROW, VACROW, RCANROW, &
...
!todo: condition for ierr.
        ...
        !> Allocate temporary variables, ME added VACROW
        allocate(ALBSROW(NA, NTYPE), CMAIROW(NA, NTYPE), GROROW(NA, NTYPE), QACROW(NA, NTYPE), VACROW(NA, NTYPE), RCANROW(NA, NTYPE), &
        ...
        do k = il1, il2
            ...
            vs%tile%uvcan(k) = VACROW(ik, jk)	! ME
        ...
        !> Deallocate temporary variables, ME added VACROW
        deallocate(ALBSROW, CMAIROW, GROROW, QACROW, VACROW, RCANROW, &
                   RHOSROW, SCANROW, SNOROW, TACROW, TBASROW, &
                   TCANROW, TPNDROW, TSNOROW, WSNOROW, ZPNDROW, &
    ...
    subroutine save_init_prog_variables_class_row(fls, shd)
        ...
        !> For SAVERESUMEFLAG 3, ME added VACROW
        real(kind = 4), dimension(:, :), allocatable :: ALBSROW, CMAIROW, GROROW, QACROW, VACROW, RCANROW, &
            RHOSROW, SCANROW, SNOROW, TACROW, TBASROW, &
            TCANROW, TPNDROW, TSNOROW, WSNOROW, ZPNDROW
        real(kind = 4), dimension(:, :, :), allocatable :: TBARROW, THICROW, THLQROW, TSFSROW
...
!todo: condition for ierr.
        ...
        !> Allocate and initialize temporary variables, ME added VACROW
        allocate(ALBSROW(NA, NTYPE), CMAIROW(NA, NTYPE), GROROW(NA, NTYPE), QACROW(NA, NTYPE), VACROW(NA, NTYPE), RCANROW(NA, NTYPE), &
                 RHOSROW(NA, NTYPE), SCANROW(NA, NTYPE), SNOROW(NA, NTYPE), TACROW(NA, NTYPE), TBASROW(NA, NTYPE), &
                 TCANROW(NA, NTYPE), TPNDROW(NA, NTYPE), TSNOROW(NA, NTYPE), WSNOROW(NA, NTYPE), ZPNDROW(NA, NTYPE), &
                 TBARROW(NA, NTYPE, NSL), THICROW(NA, NTYPE, NSL), THLQROW(NA, NTYPE, NSL), TSFSROW(NA, NTYPE, 4))
        ...
        do k = 1, shd%lc%NML
            ...
            VACROW(ik, jk) = real(vs%tile%uvcan(k), kind = 4)	! ME
