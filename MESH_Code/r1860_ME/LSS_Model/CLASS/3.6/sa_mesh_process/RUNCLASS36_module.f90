...
!> *********************************************************************
        ...
        if (ipid /= 0 .or. izero == 0) then
            ...
            cpv%VAC(il1:il2) = vs%tile%uvcan(il1:il2)	! ME
...
!         ME 30/1/2023 This block is copied over here from CLASSW 
!         to be done after the snow updates at the sub-area and tile levels for PBSM
!         It was then commented in CLASSW and now it applies here irrespective of PBSM (on or off)
!                                                                       
...
!           * THIS IS TARGETTED AS OVERLAND RUNOFF.                     
           ...
           !        cdv%TROO(I)=(cdv%TROO(I)*cdv%ROFO(I)+cpv%TSNO(I)*(SNOROF+    &  
           !                WSNROF)/ic%dts)/(cdv%ROFO(I)+(SNOROF+   &
           !                WSNROF)/ic%dts)                                       
           !        cdv%ROFO(I)=cdv%ROFO(I)+(SNOROF+WSNROF)/ic%dts              
           !        cdv%TROF(I)=(cdv%TROF(I)*cdv%ROF(I)+cpv%TSNO(I)*(SNOROF+     &
           !                WSNROF)/ic%dts)/(cdv%ROF(I)+(SNOROF+   &
           !                WSNROF)/ic%dts)                                      
           ...
           !        cdv%HTCS(I)=cdv%HTCS(I)-cpv%TSNO(I)*(SPHICE*SNOROF+SPHW*     &
           !              WSNROF)/ic%dts                                  
           !        cpv%SNO(I)=cpv%SNO(I)-SNOROF                                  
           !        cpv%WSNO(I)=cpv%WSNO(I)-WSNROF                              
           !        ZSNOW(I)=10.0                                         
           !     END IF
				!DEAL WITH VANISHINGLY SMALL SNOWPACK 
				! BLOCK existed in CLASSW for PBSM off and in REDISTRIB_SNOW for PBSM on
				! Moved here by ME on 31/1/2023 to remove duplication
				! IF(cpv%SNO(I).LT.1.0E-2 .AND. cpv%SNO(I).GT.0.0) THEN
                  ! cdv%TROO(I)=(cdv%TROO(I)*cdv%ROFO(I)+cpv%TSNO(I)*(cpv%SNO(I)+   &
                      ! cpv%WSNO(I))/ic%dts)/(cdv%ROFO(I)+(cpv%SNO(I)+cpv%WSNO(I))/ &
                      ! ic%dts)
                  ! cdv%ROFO(I)=cdv%ROFO(I)+(cpv%SNO(I)+cpv%WSNO(I))/ic%dts
                  ! cdv%TROF(I)=(cdv%TROF(I)*cdv%ROF(I)+cpv%TSNO(I)*(cpv%SNO(I)+    &
                     ! cpv%WSNO(I))/ic%dts)/(cdv%ROF(I)+(cpv%SNO(I)+cpv%WSNO(I))/   &
                     ! ic%dts)
                  ! cdv%ROF(I)=cdv%ROF(I)+(cpv%SNO(I)+cpv%WSNO(I))/ic%dts
                  ! cdv%ROFN(I)=cdv%ROFN(I)+(cpv%SNO(I)+cpv%WSNO(I))/ic%dts
                  ! cdv%PCPG(I)=cdv%PCPG(I)+(cpv%SNO(I)+cpv%WSNO(I))/ic%dts
                  ! cdv%HTCS(I)=cdv%HTCS(I)-cpv%TSNO(I)*(SPHICE*cpv%SNO(I)+SPHW*   &
                     ! cpv%WSNO(I))/ic%dts
                  ! cpv%TSNO(I)=0.0
                  ! cpv%RHOS(I)=0.0
                  ! cpv%SNO(I)=0.0                            
        ...
        vs%tile%uvcan(il1:il2) = cpv%VAC(il1:il2)	! ME
        ...
        ! ME 17/01/2023 divided Subl by the time step to be consistent with QFS
        ...
        vs%tile%lqwsice(il1:il2) = cdv%ICE(il1:il2)
        vs%tile%tice(il1:il2) = cdv%TICE(il1:il2)
