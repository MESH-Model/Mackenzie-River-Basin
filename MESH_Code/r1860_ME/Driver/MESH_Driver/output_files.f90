...
! ME added output options for canopy level temperature, humidity and wind speed
                case (VN_TACAN)
...
!                    end do						
!                case (VN_TCBOTC, 'SoilThermalConductivity_bot_canopy')
!                    do j = 1, shd%lc%IGND
!                        call output_files_append_field(fls, shd, ts, VN_TCBOTC, args, nargs, z, j)
!                    end do			
!                case (VN_TCBOTG, 'SoilThermalConductivity_bot_bare')
!                    do j = 1, shd%lc%IGND
...
!-                    call output_files_append_field(fls, shd, ts, VN_DZS, args, nargs, z)
                ...
                case (PMFRSTVN_DZAA)
                    !> User-defined temperature threshold(s)/tolerance(s) for DZAA & TZAA. / ME added TZAA on 07/09/2022
                    if (nargs > 1) then
                        do j = 2, nargs
                ...
                case (PMFRSTVN_TZAA)	!< & ME added TZAA on 07/09/2022
...
!			if ((ic%now%year /= ic%next%year .and. .not. field%in_mem .and. .not. fls_out%fclose) .or. &
        ...
        if (btest(field%ffreq, FREQ_DAILY)) then
            if (ic%now%day /= ic%next%day .or. (ic%now%year == ic%stop%year .and. & 
			   ic%now%month == ic%stop%month .and. ic%now%day /= ic%stop%day)) then
            ...
            if (((ic%now%day /= ic%next%day .or. (ic%now%year == ic%stop%year .and. & 
			   ic%now%month == ic%stop%month .and. ic%now%day /= ic%stop%day)) & 
			   .and. .not. field%in_mem .and. .not. fls_out%fclose) .or. (field%in_mem .and. fls_out%fclose)) then
                call output_files_update_file(fls, shd, FREQ_DAILY, field, field%d, fls_out%dates%d)
            end if
        end if
        if (btest(field%ffreq, FREQ_HOURLY)) then
            if (ic%now%hour /= ic%next%hour .or. (ic%now%year == ic%stop%year .and. & 
			   ic%now%month == ic%stop%month .and. ic%now%day /= ic%stop%day .and. ic%now%hour /= ic%stop%hour)) then
            ...
            if (((ic%now%hour /= ic%next%hour .or. (ic%now%year == ic%stop%year .and. & 
			   ic%now%month == ic%stop%month .and. ic%now%day /= ic%stop%day .and. ic%now%hour /= ic%stop%hour)) &
			   .and. .not. field%in_mem .and. .not. fls_out%fclose) .or. (field%in_mem .and. fls_out%fclose)) then
        ...
        !> 'Seasonal' must go last because it changes 't' regardless of the state of 'in_mem'.
        if (btest(field%ffreq, FREQ_SEASONAL)) then
            if (ic%now%month /= ic%next%month .or. (ic%now%year == ic%stop%year .and. & 
			   ic%now%month == ic%stop%month)) then
                t = ic%now%month
