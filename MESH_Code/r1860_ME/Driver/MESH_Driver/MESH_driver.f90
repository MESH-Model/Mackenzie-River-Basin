...
!-    end if !(SAVERESUMEFLAG == 2) then
    ...
    !> *********************************************************************
! Changed by ME on Sep 6, 2022 to change the stop year according to data end and then close the output files
! this ensures writing outputs for partial hour, day, month, and/or year at the end of the simulation
    if (ENDDATA) then
		call print_message('Reached end of forcing data.')
		! write(line, FMT_GEN) ic%stop%year
		! call print_message('stop year ' // line)
		! write(line, FMT_GEN) ic%now%year
		! call print_message('now year ' // line)
...
!-        end if
    ...
    ! ME commented the condition to print the estimated time per year in all cases
    ! ME updated endprog to account for portions of years
