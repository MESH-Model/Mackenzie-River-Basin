...
! ME 4/12/2021 scaling Distrib with the tile fraction according to JP's recommendation				
    ...
    if (btest(INPUTPARAMSFORMFLAG, 1) .or. btest(INPUTPARAMSFORMFLAG, 4)) then
		...
		if (pm%grid%zsnl(i) /= 0.0) pm%tile%zsnl(k) = pm%grid%zsnl(i)		!> ME 2022/09
...
! ME 4/12/2021 scaling Distrib with the tile fraction according to JP's recommendation				
    ...
    !> Moved by ME to be done before updating the soil texture for impereable layers (below SDEP)
...
!todo: Double-check this is okay with SVS.
    ...
    !> ME moved the impermeable soil layers texture update to after nudging of SDEP
    ...
    if (RUNCLASS36_flgs%PROCESS_ACTIVE) then
        ...
        !> Check for impermeable soils. ME Changes the accurcy threshold to be same as in CLASSB/BG
        do j = 2, shd%lc%IGND
