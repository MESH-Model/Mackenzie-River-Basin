module sa_mesh_run_within_tile
    ...
    subroutine run_within_tile_init(fls, shd)
        ...
        if (inp > 0) then
            ...
            ! if (allocated(vs%tile%hcps)) n = n + size(vs%tile%hcps, 2)	! ME
            ...
            if (n > 0) then
              ...
              ! if (allocated(vs%tile%hcps)) then		! ME
               ...
               ! end if
	            ...
	            ! if (allocated(vs%tile%tcbotg)) then
                    ! do j = 1, size(vs%tile%tcbotg, 2)
                        ! model_variables_to_head(n)%field, source = model_variable_pointer_1d(dat = vs%tile%tcbotg(:, j)))
                        ! n = n + 1
                    ! end do
                ! end if					
