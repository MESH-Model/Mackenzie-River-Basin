...
!-        end if
        ...
        if (count(fms%rsvr%rls%cfn == 1) > 0) then			! ME use cfn instead of b1
            ...
            if (fms%rsvr%rlsmeas%readmode /= 'n') then
                ...
                allocate(fms%rsvr%rlsmeas%val(count(fms%rsvr%rls%cfn == 1))) ! ME use cfn instead of b1
...
! ME extended the printing to all coefficients including the new outflow function
