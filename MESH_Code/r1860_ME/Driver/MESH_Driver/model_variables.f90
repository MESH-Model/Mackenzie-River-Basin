...
module model_variables
	...
	!* uvcan: Wind speed of air in the canopy. [m s**-1]. /ME 5/1/2023
        ...
        real, dimension(:), allocatable :: uvcan 	! ME 5/1/2023
...
! ME 07/09/2022 Enable if output of thermal soil properties is desired
...
!-        real, dimension(:), allocatable :: stgw
    ...
    subroutine model_variables_group_reset(group, ierr)
        ...
        if (allocated(group%uvcan)) group%uvcan = 0.0		! ME 5/1/2023
...
!-        allocate(group%pre(n), stat = z); if (z /= 0) ierr = z
        ...
        allocate(group%uvcan(n), stat = z); if (z /= 0) ierr = z	! ME 5/1/2023
