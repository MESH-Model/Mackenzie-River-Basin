...
subroutine read_reservoir_tb0(shd, iun, fname, ierr)
    ...
    call get_keyword_value(iun, vkeyword, nkeyword, ':OutflowFunction', fms%rsvr%rls%cfn, fms%rsvr%n, z)	! ME 27/4/2021
    !> Replace 'area' with 'b6' if available.
    if (any(fms%rsvr%rls%b6 > 0.0)) fms%rsvr%rls%area = fms%rsvr%rls%b6
    !> default value for OutflowFunction = Power Function if not given - ME 3/5/2021
